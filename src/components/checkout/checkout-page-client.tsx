...
export function CheckoutPageClient({ searchParams }: CheckoutPageClientProps) {
  ...
  useEffect(() => {
    // Get checkout data from searchParams prop
    const data: CheckoutData = {
      toolId: searchParams.toolId || 'pressure-washer-1',
      toolName: searchParams.toolName || 'Professional Pressure Washer',
      toolImage: '/images/placeholder-tool.jpg',
      startDate: searchParams.startDate || new Date().toLocaleDateString(),
      endDate: searchParams.endDate || new Date(Date.now() + 86400000).toLocaleDateString(),
      days: parseInt(searchParams.days || '1'),
      subtotal: parseFloat(searchParams.subtotal || '45.00'),
      taxes: parseFloat(searchParams.taxes || '3.71'),
      rentalTotal: parseFloat(searchParams.rentalTotal || '48.71'),
      deposit: parseFloat(searchParams.deposit || '100.00'),
      location: searchParams.location || 'Provo Central'
    }
