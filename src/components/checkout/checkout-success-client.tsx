...
export function CheckoutSuccessClient({ searchParams }: CheckoutSuccessClientProps) {
  ...
  useEffect(() => {
    // Get access code from searchParams or generate one
    const code = searchParams.accessCode || Math.random().toString(36).substring(2, 8).toUpperCase()
    setAccessCode(code)

    // Get rental data from searchParams
    const toolName = searchParams.toolName || 'Professional Pressure Washer'
    const startDate = searchParams.startDate || new Date().toLocaleDateString()
    const endDate = searchParams.endDate || new Date(Date.now() + 86400000).toLocaleDateString()
    const days = parseInt(searchParams.days || '1')
    const totalPaid = parseFloat(searchParams.rentalTotal || '0')
    const deposit = parseFloat(searchParams.deposit || '100')
    const location = searchParams.location || 'Provo Central'
